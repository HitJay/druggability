...
## 4. 关键证据 1：三元共存允许 (乳酸+Ag1) vs 空间冲突排斥 (AZ1+Ag1)

<div class="grid-2">
<div>

* **[乳酸 + Agonist 1] 三元共结合稳定 (物理可行)**：
  * 模拟两分子最小原子间距为 **7.71 Å**（质心相距 11.24 Å）。
  * 体系非键相互作用能达 **$-152.6$ kcal/mol**，零空间位阻重叠。
  * **药理学意义**：从微观物理上彻底证实了 **ago-PAM 协同机制**——内源乳酸与变构分子可同时结合在一枚受体上，诱发超叠加激活效应。
* **[AZ1 + Agonist 1] 空间严重排斥 (物理不可行)**：
  * 两分子在胞外前庭最小原子间距仅 **2.60 Å**（发生极度范德华重叠）。
  * 大分子 AZ1（MW 603）的尾部伸展至胞外漏斗区，物理上彻底封死了 Agonist 1 的进入通道，排除了双激动剂共存假说。
