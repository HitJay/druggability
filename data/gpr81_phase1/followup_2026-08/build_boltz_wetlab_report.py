...
def build_wetlab_subset():
    """Section 6: the 20-compound wet-lab benchmark panel (from build_wetlab_subset.py)."""
    csv_path = BASE / "wetlab_benchmark_subset.csv"
    if not csv_path.exists():
        return "<p class='muted'>subset CSV not generated yet - run build_wetlab_subset.py</p>"
    body = []
    for r in csv.DictReader(open(csv_path)):
        body.append(
            f"<tr><td><b>{esc(r['entry_id'])}</b></td>"
            f"<td>{esc(r['series'])}</td>"
            f"<td>{esc(r['ec50_nM']) or '—'}</td><td>{esc(r['emax_pct']) or '—'}</td>"
            f"<td>{esc(r['vina_tier']) or '—'}</td><td>{esc(r['boltz_tier']) or '—'}</td>"
            f"<td>{fmt(r['boltz_prob'])}</td>"
            f"<td>{esc(r['rationale'])}</td></tr>")
    return ("<table id='subset'><thead><tr><th>ID</th><th>Series</th><th>EC50 nM</th>"
            "<th>Emax%</th><th>Vina tier</th><th>Boltz tier</th><th>Boltz prob</th>"
            "<th>Selection rationale</th></tr></thead>"
...
<h2>5 · Wet-lab benchmark panel — 20-compound subset (ready for assay)</h2>
<div class="fact">Stratified panel generated by build_wetlab_subset.py: 9 leads +
2 partial-agonist probes + 3 mechanism pairs + 2 Boltz controls + 4 reference
acids. Every analog measured is counterscreened at GPR109A and GHS-R1a (the
paper's own panel). Deliberately includes Vina/Boltz tier-disagreement pairs
(c28: A/C, c35: C/A, c08: B/A) — the benchmark's core test is which layer, if
any, carries experimental signal.</div>
{subset_html}

<h2>6 · BioLib platform capabilities (candidate features)</h2>
