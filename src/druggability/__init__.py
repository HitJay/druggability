"""
druggability — 统一可药性评估与结构药理学 Agent 工具包 (Top-level Platform)

提供统一的 druggability 评估接口与多模态工具链：
- Open Targets tractability API（知识库可追踪性）
- ChEMBL ligandability proxy（已知配体覆盖度）
- fpocket 口袋检测（基于结构的分析）
- DrugCLIP 对比学习超大规模虚拟筛选
- 多肽与中分子性质及结合力评估 (druggability.peptide)
- 学术检索与报告生成 (druggability.search, druggability.report)
"""
...
from .tractability import (
    ModalityTractability,
    TractabilityResult,
    query_tractability,
    resolve_target_info,
)
