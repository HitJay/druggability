...
def _cmd_search(args: argparse.Namespace) -> None:
    from druggability.search import search
...
def _cmd_download(args: argparse.Namespace) -> None:
    from druggability.fetch import download_pdf, fetch_unpaywall_pdf_url
...
def _cmd_parse(args: argparse.Namespace) -> None:
    ...
    from druggability.parse import extract_text_pymupdf, extract_text_pdfplumber
...
def _cmd_ner(args: argparse.Namespace) -> None:
    from druggability.ner import annotate_with_pubtator
...
def _cmd_assess(args: argparse.Namespace) -> None:
    from druggability import assess_druggability
...
def _cmd_batch(args: argparse.Namespace) -> None:
    """批量评估多个靶点"""
    from druggability.batch import assess_druggability_batch
...
def _cmd_report(args: argparse.Namespace) -> None:
    ...
    from druggability.report import build_report
...
def _cmd_image2smiles(args: argparse.Namespace) -> None:
    from druggability.image2smiles import (
...
def _cmd_peptide(args: argparse.Namespace) -> None:
    ...
    from druggability.peptide import tasks as peptide_tasks_mod

    keys = [k.strip() for k in args.tasks.split(",")] if args.tasks else None

    if args.action == "report":
        import csv as _csv
        from druggability.peptide.report import build_bbb_report
        ...
        if has_pred:
            ...
            from druggability.peptide.predict import predict_bbb
    ...
    if args.action == "download":
        from druggability.peptide import datasets as peptide_datasets
    ...
    # benchmark
    from druggability.peptide import run_benchmark
